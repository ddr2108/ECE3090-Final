...
//seed = grid index of destination location (where distance = 0)
{
  ...
  #define VALID(i,j, k) \
  (s->setLocation(i,j,10*(k)), s->isValidLocation(obst,xsize,ysize))

  queue.rewind();
  while (queue.getNext(p)) {
    k=p/Z; j=(p-k*Z)/Y; i=p-j*Y-k*Z;                       //Convert 1D index to 3D indeces
    ...
    } else if (k==0 && !visited[q=p-Z+gridsize]) {        //Check for wrap around
      visited[q]=1;
      if (VALID(i,j, 35)) {dist[q]=dist[p]+1; queue.append(q);} 
    }

    if (k<zsize-1 && !visited[q=p+Z]) {   //Visit +Z neighbor
      visited[q]=1;
      if (VALID(i,j,k+1)) {dist[q]=dist[p]+1; queue.append(q);}
    } else if (k==zsize-1 && !visited[q=p+Z-gridsize]) {   //Check for wrap around
      visited[q]=1;
      if (VALID(i,j,0)) {dist[q]=dist[p]+1; queue.append(q);}
    }
  }
